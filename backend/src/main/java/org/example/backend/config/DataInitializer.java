...
public class DataInitializer {
    ...
    CommandLineRunner initData(CircuitRepository circuitRepository,
        ...
        return args -> {
            ...
            upsertCircuit(circuitRepository, "bahrain", "Bahrain (Sakhir)", 57, 22.0, 8.0, 93.0, 42.0, 0.012);
            upsertCircuit(circuitRepository, "jeddah", "Jeddah Corniche Circuit", 50, 20.0, 9.0, 90.0, 38.0, 0.008);
            upsertCircuit(circuitRepository, "melbourne", "Melbourne (Albert Park)", 58, 20.0, 8.0, 78.0, 34.0, 0.011);
            upsertCircuit(circuitRepository, "suzuka", "Suzuka Circuit", 53, 22.0, 7.0, 91.0, 32.0, 0.014);
            upsertCircuit(circuitRepository, "shanghai", "Shanghai International Circuit", 56, 23.0, 8.0, 95.0, 35.0, 0.010);
            upsertCircuit(circuitRepository, "monaco", "Monaco (Monte Carlo)", 78, 25.0, 14.0, 74.0, 30.0, 0.005);
            ...
            upsertCircuit(circuitRepository, "spielberg", "Red Bull Ring (Spielberg)", 71, 20.0, 6.0, 68.0, 40.0, 0.010);
            ...
            upsertCircuit(circuitRepository, "zandvoort", "Zandvoort (Circuit Park)", 72, 19.0, 11.0, 72.0, 32.0, 0.010);
            ...
            upsertCircuit(circuitRepository, "singapore", "Singapore (Marina Bay)", 62, 28.0, 14.0, 100.0, 32.0, 0.007);
            upsertCircuit(circuitRepository, "austin", "Austin (COTA)", 56, 20.0, 8.0, 98.0, 46.0, 0.012);
            upsertCircuit(circuitRepository, "mexico", "Mexico City (Hermanos Rodriguez)", 71, 21.0, 8.0, 79.0, 42.0, 0.010);
            upsertCircuit(circuitRepository, "interlagos", "Interlagos (Sao Paulo)", 71, 18.0, 9.0, 70.0, 38.0, 0.011);
            upsertCircuit(circuitRepository, "las-vegas", "Las Vegas Strip Circuit", 50, 20.0, 8.0, 93.0, 18.0, 0.006);
            upsertCircuit(circuitRepository, "lusail", "Lusail International Circuit", 57, 23.0, 8.0, 84.0, 44.0, 0.013);
