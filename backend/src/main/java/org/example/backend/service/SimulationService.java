...
public class SimulationService {
    ...
    // ~0.025s faster per °C below 25°C reference (more visible in UI)
    ...
    // ── Cliff saturation: prevents unrealistic degradation at very long stints ──
    // In reality, tyre degradation follows an S-curve: accelerates mid-stint then
    // plateaus as the rubber reaches a "minimum performance" floor.
    // Without saturation, the quadratic cliff term explodes for 40+ lap stints.
    private static final double CLIFF_SATURATION_LAP = 22.0; // laps past sweet spot where cliff growth halves
    ...
    // Context array indices: 0=asphaltAbrasion, 1=tyreStress, 2=lateral,
    // 3=asphaltGrip
     ...
     * @param trackEvolution  track improvement per lap from rubber (s/lap) - kept
     *                        for API compatibility
     * @param windSpeed       wind speed in km/h (0 = no wind)
     * @param windAngle       wind angle relative to main straight (0° = headwind,
     *                        180° = tailwind)
     * @param airTemp         air temperature in °C
     * @param rainIntensity   0.0 (dry) to 1.0 (heavy rain)
     * @param wetPerformance  0.0 (slick) to 1.0 (full wet) — how well the tyre
     *                        handles rain
     * @param drivingMode     driving aggression level (CONSERVATIVE/NORMAL/PUSH)
     * @param totalRaceLaps   total number of laps in the race (for fuel
     *                        calculation)
     */
    public double calculateLapTime(double baseLapTime,
        ...
        // Cooling is now dynamic: airTemp affects how fast tyres shed heat
        ...
        // Dynamic cooling: more cooling in cold air, less in heat
        // Base cooling is 2.0s at 25°C air temp
        ...
        // Lap 7+: Full degradation with saturating cliff (S-curve)
        double wearEffect;
        if (lapOnTyre <= 2) {
            ...
            // Saturating cliff: effectiveLap² / (1 + effectiveLap/saturation)
            // At effectiveLap=10: 100/1.45 = 69 (mild reduction)
            // At effectiveLap=20: 400/1.91 = 209 (significant reduction)
            // At effectiveLap=40: 1600/2.82 = 567 (prevents 3-stop bias)
            double saturatedCliff = Math.pow(effectiveLap, 2.0)
                    / (1.0 + effectiveLap / CLIFF_SATURATION_LAP);
            wearEffect = (baseWear
                    + (degBase * effectiveLap) // Linear part
                    + (degBase * 0.12 * saturatedCliff)) // Saturating cliff
        ...
        // Industry standard: ~0.3s per 10kg. With 1.5-2kg/lap consumption =
        // 0.07-0.1s/lap.
     ...
     *
     * @param baseDeg compound degradation coefficient (used to identify compound
     *                type)
     ...
     *
     * @return multiplier where 1.0 is average, <1.0 is gentle on tyres, >1.0 is
     *         aggressive
     */
